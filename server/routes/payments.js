...
// Dedicated Available Credit balance store. This is the source of truth for
// the current balance so a credit that has been consumed can never be
// recreated from an old payment's amount/allocation values.
const paymentCreditBalanceSchema = new mongoose.Schema({
  clientId: { type: mongoose.Schema.Types.ObjectId, required: true, index: true },
  clientType: { type: String, enum: ['Customer', 'Vendor'], required: true, index: true },
  availableCredit: { type: Number, default: 0, min: 0 }
}, { timestamps: true });
paymentCreditBalanceSchema.index({ clientId: 1, clientType: 1 }, { unique: true });
const PaymentCreditBalance = mongoose.models.PaymentCreditBalance ||
  mongoose.model('PaymentCreditBalance', paymentCreditBalanceSchema, 'paymentcreditbalances');
...
async function getClientCreditBalance(clientId, clientType, excludePaymentId) {
  const existingBalance = await PaymentCreditBalance.findOne({ clientId, clientType }).lean();
  if (existingBalance) {
    return roundMoney(Math.max(0, Number(existingBalance.availableCredit) || 0));
  }

  // One-time bootstrap for customers/vendors created before the dedicated
  // balance store existed. After this point, ONLY the balance document is
  // used; old payment amounts are never re-read to recreate credit.
  const query = { clientId, clientType };
  if (excludePaymentId) query._id = { $ne: excludePaymentId };
  const rows = await Payment.find(query).select('availableCreditGenerated availableCreditApplied').lean();
  let generated = 0;
  let applied = 0;
  for (const row of rows) {
    generated += Math.max(0, Number(row?.availableCreditGenerated) || 0);
    applied += Math.max(0, Number(row?.availableCreditApplied) || 0);
  }
  const balance = roundMoney(Math.max(0, generated - applied));
  await PaymentCreditBalance.updateOne(
    { clientId, clientType },
    { $setOnInsert: { clientId, clientType, availableCredit: balance } },
    { upsert: true }
  );
  const saved = await PaymentCreditBalance.findOne({ clientId, clientType }).lean();
  return roundMoney(Math.max(0, Number(saved?.availableCredit ?? balance) || 0));
}

async function setClientCreditBalance(clientId, clientType, value) {
  const balance = roundMoney(Math.max(0, Number(value) || 0));
  await PaymentCreditBalance.updateOne(
    { clientId, clientType },
    { $set: { availableCredit: balance } },
    { upsert: true }
  );
  return balance;
}

async function adjustClientCreditBalance(clientId, clientType, delta) {
  const current = await getClientCreditBalance(clientId, clientType);
  return setClientCreditBalance(clientId, clientType, current + Number(delta || 0));
}
...
const calculateAvailableCreditApplied = ({ paymentAmount, allocationAmount, availableCredit, applyAvailableCredit }) => {
  if (!applyAvailableCredit) return 0;
  const payment = Math.max(0, Number(paymentAmount) || 0);
  const allocation = Math.max(0, Number(allocationAmount) || 0);
  const credit = Math.max(0, Number(availableCredit) || 0);
  return roundMoney(Math.min(payment, allocation, credit));
};
...
if (!paymentNumber) paymentNumber = await getNextPaymentNumber();
const clientId = req.body.clientId;
const clientType = req.body.clientType || 'Customer';
const paymentDate = req.body.paymentDate;
const amount = Math.max(0, Number(req.body.amount) || 0);
const description = req.body.description || '';
const autoAllocateOnSubmit = req.body.autoAllocateOnSubmit === true;
const applyAvailableCredit = req.body.applyAvailableCredit === true;
...
if (applyAvailableCredit && requestedAllocations.length === 0 && !autoAllocateOnSubmit) {
return res.status(400).json({ message: 'Select at least one invoice when applying Available Credit.' });
}
if (!(amount > 0) && !(applyAvailableCredit && requestedAllocations.length > 0)) {
return res.status(400).json({ message: 'Payment amount must be greater than 0' });
}
const allocationResult = requestedAllocations.length > 0
? await allocatePaymentByRequestedAmounts({ clientId, clientType, requestedAllocations })
: autoAllocateOnSubmit
? await allocatePaymentToInvoices({ clientId, clientType, amount: sourceAmount, invoiceOrder })
: { allocations: [], appliedAmount: 0 };
const { allocations, appliedAmount } = allocationResult;
if (requestedAllocations.length > 0 || autoAllocateOnSubmit) {
if (!(appliedAmount > 0)) {
return res.status(400).json({ message: 'No pending invoices available to apply this payment.' });
}
if (appliedAmount > sourceAmount) {
return res.status(400).json({ message: 'Invoice allocation cannot exceed payment amount and available credit.' });
}
}
const finalAmount = roundMoney(amount);
const availableCreditApplied = calculateAvailableCreditApplied({
paymentAmount: finalAmount,
allocationAmount: appliedAmount,
availableCredit,
applyAvailableCredit
});
const availableCreditGenerated = applyAvailableCredit
? 0
: roundMoney(Math.max(0, finalAmount - appliedAmount));
...
updateOps.$set = { ...(updateOps.$set || {}), availableCreditGenerated, availableCreditApplied };
...
// New payment either creates surplus credit or consumes existing credit.
await adjustClientCreditBalance(
  clientId,
  clientType,
  availableCreditGenerated - availableCreditApplied
);
const balanceAfter = await getClientCreditBalance(clientId, clientType);
payment.availableCreditBalance = balanceAfter;
const saved = await payment.save();
res.status(201).json({ ...saved.toObject(), availableCredit: balanceAfter });
} catch (err) {
if (err.code === 11000) {
...
const amount = Math.max(0, Number(req.body.amount ?? existing.amount) || 0);
...
if (!clientId) return res.status(400).json({ message: 'Client is required' });
if (!paymentDate) return res.status(400).json({ message: 'Payment date is required' });
const currentStoredBalance = await getClientCreditBalance(clientId, clientType);
const oldCreditEffect = Number(existing.availableCreditGenerated || 0) - Number(existing.availableCreditApplied || 0);
const creditBalance = await setClientCreditBalance(clientId, clientType, currentStoredBalance - oldCreditEffect);
const availableCredit = Math.max(0, creditBalance);
const sourceAmount = roundMoney(amount);
if (applyAvailableCredit && requestedAllocations.length === 0 && !autoAllocateOnSubmit) {
return res.status(400).json({ message: 'Select at least one invoice when applying Available Credit.' });
...
recordChange('availableCreditGenerated', existing.availableCreditGenerated, availableCreditGenerated);
recordChange('availableCreditApplied', existing.availableCreditApplied, availableCreditApplied);
...
existing.availableCreditGenerated = availableCreditGenerated;
existing.availableCreditApplied = availableCreditApplied;
...
const balanceAfter = await getClientCreditBalance(clientId, clientType);
existing.availableCreditBalance = balanceAfter;
const updated = await existing.save();
res.json({ ...updated.toObject(), availableCredit: balanceAfter });
} catch (err) {
if (err.code === 11000) {
return res.status(400).json({ message: 'Payment number must be unique. A payment with this number already exists.' });
}
...
const paymentsToDelete = await Payment.find({ _id: { $in: ids } }).select('clientId clientType availableCreditGenerated availableCreditApplied').lean();
await Payment.deleteMany({ _id: { $in: ids } });
for (const row of paymentsToDelete) {
  const effect = Number(row.availableCreditGenerated || 0) - Number(row.availableCreditApplied || 0);
  await adjustClientCreditBalance(row.clientId, row.clientType, -effect);
}
...
try {
const payment = await Payment.findById(req.params.id).select('clientId clientType availableCreditGenerated availableCreditApplied');
if (!payment) return res.status(404).json({ message: 'Payment not found' });
const effect = Number(payment.availableCreditGenerated || 0) - Number(payment.availableCreditApplied || 0);
await Payment.findByIdAndDelete(req.params.id);
await adjustClientCreditBalance(payment.clientId, payment.clientType, -effect);
