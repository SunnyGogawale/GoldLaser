...
function PurchasePayment() {
  ...
  const handleCsvFileSelection = async (event) => {
    ...
    if (!csvSelectedClientId) {
      setCsvImportError('Please select a vendor before uploading a CSV file.')
      return
    }
    ...
    try {
      ...
      if (!parseCsvText(text).some((row) => row.some((value) => String(value).trim() !== ''))) {
        throw new Error('The selected file does not contain any usable rows.')
      }
      setCsvImportFileName(file.name)
      setCsvHeaders(parsed.headers)
      setCsvDataRows(parsed.dataRows)
      setCsvFieldMapping({
        paymentNumber: getSuggestedCsvHeader(parsed.headers, ['payment number', 'payment no', 'payment']) || '',
        paymentDate: getSuggestedCsvHeader(parsed.headers, ['payment date', 'date']) || '',
        amount: getSuggestedCsvHeader(parsed.headers, ['amount', 'amount due', 'total']) || '',
        description: getSuggestedCsvHeader(parsed.headers, ['description', 'narration', 'remarks', 'note']) || ''
      })
      showSuccessToast(`Loaded ${parsed.dataRows.length} row(s) from ${file.name}`)
    } catch (error) {
  ...
  const handleCsvImportSubmit = async () => {
    if (!csvSelectedClientId || !csvDataRows.length) {
      setCsvImportError(!csvSelectedClientId ? 'Please select a vendor before importing the CSV file.' : 'Please choose a CSV file first.')
      return
    }
    try {
      setCsvImporting(true)
