                  ...
                  <tbody>
                    {invoices.map((invoice) => (
                      <tr key={invoice._id} style={{ borderBottom: '1px solid var(--border)' }}>
                        <td style={{ padding: '0.75rem 0.5rem' }}>{invoice.invoiceNumber}</td>
                        <td style={{ padding: '0.75rem 0.5rem' }}>
                          {(invoice.customerId?.customerName || `${invoice.customerId?.firstName || ''} ${invoice.customerId?.lastName || ''}`.replace(/\s+/g, ' ').trim() || 'Unknown')}{invoice.customerId?.id ? ` (${invoice.customerId.id})` : ''}
                        </td>
                        <td style={{ padding: '0.75rem 0.5rem' }}>{new Date(invoice.invoiceDate).toLocaleDateString()}</td>
                        <td style={{ padding: '0.75rem 0.5rem' }}>₹{invoice.totalAmount.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                        <td style={{ padding: '0.75rem 0.5rem' }}>
